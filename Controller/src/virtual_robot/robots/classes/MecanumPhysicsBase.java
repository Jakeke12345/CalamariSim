...
public abstract class MecanumPhysicsBase extends VirtualBot {
    ...
    public MecanumPhysicsBase(MotorType driveMotorType){
        this(driveMotorType, true, true,
                new String[]{"back_left_motor", "front_left_motor", "front_right_motor", "back_right_motor"}
        );
    }

    public MecanumPhysicsBase(boolean useDist, boolean useColor, String[] motorNames){
        this(MotorType.Neverest40, useDist, useColor, motorNames);
    }

    private MecanumPhysicsBase(MotorType driveMotorType, boolean useDist, boolean useColor, String[] motorNames){
        super();
    ...
    public void setEncoders(DeadWheelEncoder... encoders){
    ...
    public void initialize(){
        ...
        motors = new DcMotorExImpl[]{
                (DcMotorExImpl) hardwareMap.get(DcMotorEx.class, MOTOR_NAMES[0]),
                (DcMotorExImpl) hardwareMap.get(DcMotorEx.class, MOTOR_NAMES[1]),
                (DcMotorExImpl) hardwareMap.get(DcMotorEx.class, MOTOR_NAMES[2]),
                (DcMotorExImpl) hardwareMap.get(DcMotorEx.class, MOTOR_NAMES[3])
        };
        if (USE_DIST) {
    ...
    protected void createHardwareMap() {
        ...
        for (int i=0; i<4; i++){
            hardwareMap.put(MOTOR_NAMES[i], new DcMotorExImpl(MOTOR_TYPE, motorController0, i, encoders[i]));
        }
        if(USE_DIST) {
        ...
        if (USE_COLOR) hardwareMap.put("color_sensor", controller.new ColorSensorImpl());
    ...
    public synchronized void updateStateAndSensors(double millis) {
        ...
        if (USE_COLOR) colorSensor.updateColor(x, y);
